...
class UploadOrCamera extends StatefulWidget {
  ...
  final Map<String, Map<String, String>> existingComponentImages;
  final List<String> existingDetections;
  final List<int> existingBatch;

  const UploadOrCamera({
    super.key, 
    required this.category, 
    this.fromChatbotRedo = false,
    this.existingComponentImages = const {},
    this.existingDetections = const [],
    this.existingBatch = const [],
  });
    ...
    return WillPopScope(
      onWillPop: () async {
        if (widget.fromChatbotRedo) {
          // Return to ChatbotRedo with existing data
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (context) => ChatbotRedo(
                ...
                initialDetections: widget.existingDetections,
                initialComponentImages: widget.existingComponentImages,
                initialBatch: widget.existingBatch,
              ),
            ),
          );
          return false;
                ...
                GestureDetector(
                  onTap: () async {
                    ...
                    if (results.isNotEmpty) {
                      // Extract new detections and component images
                      ...
                      // Create new batch numbers starting after the highest existing batch number
                      int startBatchNum = 0;
                      if (widget.existingBatch.isNotEmpty) {
                        startBatchNum = widget.existingBatch.reduce((a, b) => a > b ? a : b) + 1;
                      }
                      final List<int> newBatch = List.generate(results.length, (i) => i + startBatchNum);
                      
                      // Combine existing and new data
                      final combinedDetections = [...widget.existingDetections, ...newDetections];
                      final combinedComponentImages = Map<String, Map<String, String>>.from(widget.existingComponentImages);
                      combinedComponentImages.addAll(newComponentImages);
                      final combinedBatch = [...widget.existingBatch, ...newBatch];

                      // Navigate back to ChatbotRedo with all data
                      Navigator.pushReplacement(
                        context,
                        MaterialPageRoute(
                          builder: (context) => ChatbotRedo(
                            initialCategory: widget.category,
                            initialImagePath: null,
                            initialDetections: combinedDetections,
                            initialComponentImages: combinedComponentImages,
                            initialBatch: combinedBatch,
                          ),
                        ),
                      );
                    } else {
